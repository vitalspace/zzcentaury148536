...
spotify.render_edit_user = (req, res) => {
  const user_data = getConnection().get('spotify').find({ id: req.params.id }).get('accounts').find({ id: req.params.did }).value()
  res.render('spotify_user/edit_user', {
    user_data,
    id: req.params.id
  })
}

spotify.edit_user = async (req, res) => {
  req.flash('api_success', 'User successfully modified.')
  const { did, name, img } = req.body
  const id = did
  const obj = { id, name, img }
  await getConnection().get('spotify').find({ id: req.body.id }).get('accounts').find({ id: req.body.did })
  .assign(obj)
  .write()
  res.redirect('/spotify')
}
